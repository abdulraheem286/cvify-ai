...
// The id lets Phase 4 (PDF export) grab exactly this element.
export function CvDocument({ cv }: { cv: CVResult }) {
  return (
    <div
      id="cv-document"
      className="mx-auto w-full max-w-[800px] bg-white text-zinc-800 shadow-2xl"
    >
      <div className="p-10 sm:p-14">
        {/* Header */}
        <header className="border-b-2 border-emerald-500 pb-5">
          ...
          <Section title="Skills">
            <div className="flex flex-wrap gap-2">
              {cv.skills.map((s, i) => (
                <span
                  key={i}
                  className="rounded-md bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 ring-1 ring-emerald-200"
                >
                  {s}
                </span>
              ))}
