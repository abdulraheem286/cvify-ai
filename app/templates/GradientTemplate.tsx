...
// Gradient: bold primary→secondary gradient hero, two-column body.
export function GradientTemplate({ cv, domId = "cv-document", theme = DEFAULT_THEME, datePlacement = "below" }: { cv: CVResult; domId?: string; theme?: Theme; datePlacement?: DatePlacement }) {
  ...
  return (
    ...
    >
      <header
        className="flex items-center gap-6 p-8 text-white sm:p-10"
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-lg text-white/85">{cv.jobTitle}</p>
            ...
            <Section title="Experience">
              {cv.experience.map((job, i) => (
                <div key={i} className="mb-4">
                  <h3 className="font-semibold text-zinc-900">{job.role}<WorkTag mode={job.workMode} className="text-zinc-500" />{!job.company && <InlineDate period={job.period} type={job.employmentType} placement={datePlacement} className="text-zinc-500" />}</h3>
            ...
            <Section title="Education">
              {cv.education.map((ed, i) => (
                <div key={i} className="mb-2">
                  <p className="text-sm font-semibold text-zinc-900">{ed.degree}{!ed.institution && <InlineDate period={ed.period} placement={datePlacement} className="text-zinc-400" />}</p>
                  {ed.institution && (
          ...
          {cv.skills?.length > 0 && (
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {cv.skills.map((s, i) => (
                  <span key={i} className="rounded bg-white px-2 py-1 text-xs text-zinc-700 ring-1 ring-[var(--primary)]/20">{s}</span>
                ))}
              </div>
            </div>
          )}
