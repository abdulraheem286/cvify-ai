...
export function MinimalTemplate({ cv, domId = "cv-document", theme = DEFAULT_THEME, datePlacement = "below" }: Props) {
  const contactLine = [
    cv.contact?.email,
    cv.contact?.phone,
    cv.contact?.location,
            ...
            {cv.experience.map((job, i) => (
              <div key={i} className="mb-4">
                <h3 className="font-semibold text-zinc-900">
                  {job.role}
                  <WorkTag mode={job.workMode} className="text-zinc-400" />
                  {job.company && <span className="font-normal text-zinc-600"> — {job.company}</span>}
                  <InlineDate period={job.period} type={job.employmentType} placement={datePlacement} className="text-zinc-400" />
                </h3>
        ...
        {cv.customSections?.map((s, ci) =>
          s.heading && s.items?.length ? (
            <Section key={`cs-${ci}`} title={s.heading}>
              <CustomItems items={s.items} center datePlacement={datePlacement} />
            </Section>
          ) : null,
        )}
      </div>
