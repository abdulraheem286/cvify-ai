...
export function HairlineTemplate({ cv, domId = "cv-document", theme = DEFAULT_THEME, datePlacement = "below" }: { cv: CVResult; domId?: string; theme?: Theme; datePlacement?: DatePlacement }) {
          ...
          <Section title="Experience">
            {cv.experience.map((job, i) => (
              <div key={i} className="mb-4 last:mb-0">
                <h3 className="font-semibold text-zinc-900">{job.role}<WorkTag mode={job.workMode} className="text-zinc-400" />{job.company && <span className="font-normal text-zinc-600"> — {job.company}</span>}<InlineDate period={job.period} type={job.employmentType} placement={datePlacement} className="text-zinc-400" /></h3>
                <StackedDate period={job.period} type={job.employmentType} placement={datePlacement} className="text-zinc-400" />
