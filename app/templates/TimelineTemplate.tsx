      ...
      <div className="p-10 sm:p-12">
        <header className="flex items-center gap-5">
          {cv.photo && (
            ...
            <img src={cv.photo} alt="" className="h-20 w-20 shrink-0 rounded-2xl object-cover ring-2 ring-[var(--primary)]/30" />
          )}
          <div>
        ...
        {cv.experience?.length > 0 && (
          <Section title="Experience">
            <div className="relative ml-1 border-l-2 border-[var(--primary)]/30 pl-6">
        ...
        {cv.skills?.length > 0 && (
          <Section title="Skills">
            <div className="flex flex-wrap gap-2">
              {cv.skills.map((s, i) => (
                <span key={i} className="rounded-full bg-[var(--primary)]/10 px-3 py-1 text-xs font-medium text-[var(--primary)]">{s}</span>
              ))}
            </div>
          </Section>
        )}
