...
export function UnderlineTemplate({ cv, domId = "cv-document", theme = DEFAULT_THEME, datePlacement = "below" }: { cv: CVResult; domId?: string; theme?: Theme; datePlacement?: DatePlacement }) {
  ...
  return (
    ...
    >
      <div className="p-10 sm:p-14">
        <header>
          <h1 className="inline-block font-[family-name:var(--font-heading)] text-4xl font-bold tracking-tight text-[var(--secondary)]">
            {cv.fullName}
          </h1>
          <div className="mt-1 h-1 w-16 rounded bg-[var(--primary)]" />
          <p className="mt-3 text-lg font-medium text-[var(--primary)]">{cv.jobTitle}</p>
          ...
          <Section title="Education">
            {cv.education.map((ed, i) => (
              <div key={i} className="mb-2">
                <p className="text-sm"><span className="font-semibold text-zinc-900">{ed.degree}</span>{ed.institution && <span className="text-zinc-600"> — {ed.institution}</span>}<InlineDate period={ed.period} placement={datePlacement} className="text-zinc-500" /></p>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mt-6">
      <h2 className="mb-3 inline-block border-b-2 border-[var(--primary)] pb-0.5 font-[family-name:var(--font-heading)] text-sm font-bold uppercase tracking-wider text-zinc-900">{title}</h2>
      {children}
    </section>
  );
}
